# Universal Quantum Foam Hypothesis (UQSH)

A field-based hypothesis that mechanically explains all physical forces.

**Author:** Ukshin Q. Rexhepi | Independent Researcher, Tübingen, Germany  
**GitHub:** [Universal-Quantum-Foam-Hypothesis-UQSH-](https://github.com/ukshinrexhepi-cloud/Universal-Quantum-Foam-Hypothesis-UQSH-)  
**ORCID:** [0009-0003-4145-5431](https://orcid.org/0009-0003-4145-5431)

---

## Overview

The UQSH describes physical reality as a continuous dynamic field medium.
Observable matter, radiation, gravitation, and dark matter effects are
interpreted as different organisational states of the same underlying field.

## Papers

All papers are listed on ORCID and available as preprints.  
See: [orcid.org/0009-0003-4145-5431](https://orcid.org/0009-0003-4145-5431)

The most recent empirical paper is available on Preprints.org:  
[Bimodal Regime Structure in Galactic Rotation Curves](https://www.preprints.org/manuscript/206674)

## Related Repository

SPARC data analysis:  
[dm-effect-analysis](https://github.com/ukshinrexhepi-cloud/dm-effect-analysis)
